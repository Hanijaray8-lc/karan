...
const ClientDetails = () => {
  ...
  // Determine weekly due amount from client record.  When `weekly_amount`
  // is present we trust it; otherwise fall back to pending/total_weeks and
  // finally the old defaults.
  ...
  const handleClientSelect = async (client) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:5000/api/payments/client/${client._id}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      // Load payments for the selected client
      const payments = await fetchPaymentHistory(client._id, clientData);
      setDuePayments(payments);
    } catch (error) {
