...
const navItems = [
  ...
  { label: 'Approvals', path: '/Manager/ManagerAproval', isApproval: true },
  { label: 'Add Client', path: '/Manager/MAddClient' },
  { label: 'Client Profile', path: '/Manager/MClientDetails' },
  { label: 'Transfer Clients', path: '/Manager/ClientManage' },
  { label: 'Weekly Dues', path: '/Manager/MDailyDues' },
  { label: 'Payment History', path: '/Manager/MpaymentHistory' },
  { label: 'Pending Clients', path: '/Manager/MPendingClients' },
  { label: 'Active / Inactive', path: '/Manager/Acive-Inactive' },
];
...
const ManagerNavbar = () => {
  ...
  return (
    <>
      ...
      <nav className="sticky top-0 z-50 bg-emerald-900 text-white w-full shadow-md" style={{ paddingTop: 'env(safe-area-inset-top, 0px)' }}>
        <div className="px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex items-center space-x-3">
                  ...
                  <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden z-50 text-gray-800 animate-fadeIn">
                    ...
                    <div className="p-2 bg-gray-50 text-center border-t border-gray-100">
                      <Link
                        to="/Manager/ManagerAproval"
                        onClick={() => setShowNotificationDropdown(false)}
                        className="text-xs font-bold text-emerald-800 hover:underline block py-1"
                      >
              ...
              {/* Manager User Info */}
              <div className="flex items-center space-x-2.5 p-1 rounded-xl">
                <div className="w-8 h-8 rounded-full bg-emerald-700 flex items-center justify-center font-bold text-sm shadow-sm overflow-hidden">
          ...
          {isLogoutModalOpen && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 px-4">
              <div className="bg-white rounded-2xl p-6 max-w-sm w-full text-center shadow-2xl text-gray-800">
          ...
          {/* Mobile Menu */}
          {isMobileMenuOpen && (
            <div className="md:hidden bg-emerald-800 py-4 px-2 space-y-2 border-t border-emerald-700" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
              {navItems.map((item) => {
                const isActive = location.pathname.toLowerCase() === item.path.toLowerCase() ||
                  (item.isApproval && (location.pathname.toLowerCase().includes('approval') || location.pathname.toLowerCase().includes('aproval')));
