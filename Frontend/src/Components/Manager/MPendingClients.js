...
const MPendingClients = () => {
  ...
  const handleExportPDF = async () => {
    ...
    const getPageMaxBottom = (pageEl) => {
      const pageRect = pageEl.getBoundingClientRect();
      const paddingBottomPx = pageRect.height * (25 / 297);
      return pageRect.bottom - paddingBottomPx;
    };
    ...
    const createGroupSection = (landmark, district, isContinued = false) => {
      const lmDiv = document.createElement('div');
      lmDiv.style.marginBottom = '20px';

      const lmTitle = document.createElement('h3');
      lmTitle.innerText = `${landmark} (${district})${isContinued ? ' — Continued' : ''}`;
    ...
    sortedGroupKeys.forEach(groupKey => {
      ...
      let { lmDiv, table: currentTable } = createGroupSection(landmark, district, false);
      currentPage.appendChild(lmDiv);

      let serialNo = 1;
      let rowsInGroupOnCurrentPage = 0;
      ...
      groupClients.forEach(client => {
        ...
        const maxBottom = getPageMaxBottom(currentPage);
        const trRect = tr.getBoundingClientRect();

        if (trRect.bottom > maxBottom - 0.5) {
          if (rowsInGroupOnCurrentPage === 0 && currentPage.children.length > 2) {
            currentPage.removeChild(lmDiv);

            currentPage = createNewPage();
            pages.push(currentPage);
            createHeader(currentPage, false);

            const newSec = createGroupSection(landmark, district, false);
            lmDiv = newSec.lmDiv;
            currentTable = newSec.table;
            currentPage.appendChild(lmDiv);

            currentTable.appendChild(tr);
            rowsInGroupOnCurrentPage = 1;
          } else {
            ...
            const newSec = createGroupSection(landmark, district, true);
            lmDiv = newSec.lmDiv;
            currentTable = newSec.table;
            currentPage.appendChild(lmDiv);
    ...
    const summaryRect = summaryDiv.getBoundingClientRect();
    const maxBottom = getPageMaxBottom(currentPage);

    if (summaryRect.bottom > maxBottom - 0.5) {
                ...
                <tbody className="divide-y divide-gray-100 text-sm text-gray-700">
                  {filteredList.map((client, idx) => {
                    ...
                    return (
                      <tr
                        key={client._id}
                        onClick={() => navigate(`/Manager/MClientDetails?clientId=${client._id}`)}
                        ...
                        <td className="px-5 py-4 text-center">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                navigate(`/Manager/MClientDetails?clientId=${client._id}`);
                              }}
...
export default MPendingClients;
