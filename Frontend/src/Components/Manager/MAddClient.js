...
export default function MClientManagement() {
  ...
  const handleAddClient = async (e) => {
    ...
    try {
      ...
      // include computed weekly fields so backend can persist fixed weekly amount (e.g., ₹575)
  ...
  const handleEditClient = async (e) => {
    ...
    try {
      ...
      // include weekly fields when updating so admin/manager edits persist them
  ...
  const handleDeleteClient = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = (data && data.message) ? data.message : `Failed to delete (status ${res.status})`;
        throw new Error(msg);
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#f8fafc] to-[#e9f0f5] pb-12">
      ...
      <header className="relative mt-2 mx-4 rounded-2xl overflow-hidden backdrop-blur-lg bg-gradient-to-r from-[#16423C] to-[#1f5a52] shadow-xl mb-6">
        ...
        <div className="relative px-6 py-6 text-white">
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight">Client Management (Manager)</h1>
