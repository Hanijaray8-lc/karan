...
export default function ManagerAproval() {
  ...
  const fetchApprovalsData = async (isManual = false) => {
    try {
      ...
      if (res.data && res.data.success) {
        ...
        // Alert on new incoming agent login attempt
        if (pAgents.length > prevPendingCountRef.current && prevPendingCountRef.current !== 0) {
          showToast(`🔔 Alert: A new agent is waiting for Manager approval!`, 'alert');
        }
  ...
  const handleApprove = async (agent) => {
    try {
      ...
      if (res.data && res.data.success) {
        showToast(`✅ Agent ${agent.name || agent.username} approved! Access unlocked.`, 'success');
        fetchApprovalsData();
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        {/* Top Header Banner */}
        <div className="bg-gradient-to-r from-teal-900 via-emerald-900 to-[#16423c] text-white rounded-3xl p-6 sm:p-8 shadow-xl relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div className="space-y-2">
              <div className="inline-flex items-center gap-2 bg-emerald-800/80 px-3.5 py-1 rounded-full text-xs font-semibold text-emerald-200 border border-emerald-700/60">
                ...
                <span>Manager Approval Portal</span>
              </div>
              <h1 className="text-3xl font-extrabold tracking-tight">Agent Access & Approvals</h1>
              <p className="text-emerald-100/90 text-sm max-w-xl">
                Authorize agent logins in real-time. When an agent attempts to log in, you can verify and grant access with a single click.
              </p>
            </div>
        ...
        {/* Live Alert Notice Banner (if any agent is waiting) */}
            ...
            <div className="flex items-start sm:items-center gap-3.5">
              ...
              <div>
                <h3 className="font-bold text-amber-950 text-base">
                  {stats.pendingCount} Agent{stats.pendingCount > 1 ? 's' : ''} Awaiting Approval
                </h3>
                <p className="text-amber-800 text-xs mt-0.5">
                  Agents trying to log in are currently waiting on the approval screen.
                </p>
              </div>
            </div>
            <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Waiting Approval</span>
          ...
          >
            ...
            <div className="mt-3 flex items-baseline gap-2">
              ...
              <span className="text-xs font-semibold text-gray-500">login attempts</span>
            </div>
          </div>
          ...
          >
            ...
            <div className="mt-3 flex items-baseline gap-2">
              ...
              <span className="text-xs font-semibold text-gray-500">active agents</span>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl p-4 border border-gray-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 text-xs font-medium rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-700 focus:border-transparent transition bg-gray-50/50"
            />
          </div>
        </div>
        ...
        {loading ? (
          <div className="bg-white rounded-3xl p-12 border border-gray-100 shadow-sm flex flex-col items-center justify-center space-y-4">
            ...
            <p className="text-sm text-gray-500 font-medium">Loading agent approval queue...</p>
          </div>
          ...
          <div className="bg-white rounded-3xl p-12 border border-gray-100 shadow-sm text-center space-y-3">
            ...
            <p className="text-xs text-gray-500 max-w-md mx-auto">
              {activeTab === 'pending'
                ? 'All agent login requests are up to date. Incoming agent login attempts will show here in real-time.'
                  ...
                  <div className="p-6 space-y-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3.5">
                        <div className="w-12 h-12 rounded-2xl bg-emerald-800 text-white flex items-center justify-center font-bold text-lg shadow-md overflow-hidden shrink-0">
                          {agent.profilePhoto ? (
                            <img
