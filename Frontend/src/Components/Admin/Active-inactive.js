...
import { User, Search, Filter, MapPin, Phone, CheckCircle, Clock, ShieldAlert, Award, Calendar, RefreshCw } from 'lucide-react';

const AdminActiveInactive = () => {
  ...
  const fetchClients = async () => {
    try {
      ...
      console.error('Error fetching clients for Admin Active/Inactive:', err);
  ...
  const is12WeekFullyCompleted = (client) => {
    ...
    // Inactive side ONLY shows clients who have actually completed all 12 weeks of payments (weeksPaid >= totalWeeks)
        ...
        <div className="bg-gradient-to-r from-emerald-800 to-teal-900 rounded-2xl p-6 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight flex items-center gap-3">
              <span>Admin Client Status Dashboard</span>
...
export default AdminActiveInactive;
