...
const DailyDues = () => {
  ...
  const fetchClientsWithoutLoading = async () => {
    try {
      ...
      if (data.success && data.clients) {
        ...
        // Try to fetch payment records to mark which clients are paid today
        try {
          ...
          // update serverPaidTodayMap for modal checks
          const paidMap = {};
  ...
  const fetchClients = async () => {
    ...
    try {
      ...
      if (data.success && data.clients) {
        ...
        // Fetch payments to determine paid status for today
        try {
          const payRes = await fetch('http://localhost:5000/api/payments/test/all');
          const payJson = payRes.ok ? await payRes.json() : null;
          const payments = (payJson && payJson.data && payJson.data.payments) || [];
  ...
  const calculateStats = () => {
    ...
    // If a calendar date is selected, filter to only clients due on that date
    if (calendarDateFilter) {
  ...
  const checkServerPaymentsToday = async (clientId) => {
    try {
      ...
      const end = todayKey();
      const res = await fetch(`http://localhost:5000/api/payments/history?clientId=${clientId}&startDate=${start}&endDate=${end}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleNotPaid = async (client) => {
    try {
      ...
      if (currentEnd && !isNaN(currentEnd)) {
        ...
        const start = new Date(client.loan_start_date);
        newEnd = new Date(start.getTime() + 13 * 7 * 24 * 60 * 60 * 1000); // push to 13 weeks
      } else {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#E9EFEC] p-4 md:p-5 pt-28 max-w-[1400px] mx-auto font-sans mt-10">
      {/* Notification */}
      {notification && (
        <div className={`
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {filteredClients.map((client) => (
                <div
                  ...
                  className="bg-white rounded-xl p-5 shadow-lg border-2 border-transparent hover:border-[#C4DAD2] hover:-translate-y-1 hover:shadow-xl transition-all cursor-pointer flex flex-col h-full active:scale-95"
                >
          ...
          >
            ...
            <div className="p-6">
              ...
              <button
                ...
                disabled={!selectedClient || notPaidTodayMap[selectedClient._id]}
